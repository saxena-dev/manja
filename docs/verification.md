...
| `http_order_rows_span` | `http` | a strict rejection ends its operation span as `decode_error`, and its attempt span records `error_class` `decode_error`. It is alone in its binary, because a concurrent test without a subscriber could otherwise have the span cached as disabled |
