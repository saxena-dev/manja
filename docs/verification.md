...
| `http_mutual_funds` | `http` | mutual fund orders, one order by UUID, SIPs, holdings and the instrument list decoded field by field from the official samples; an unsafe order ID sending nothing; a malformed CSV row; an unknown status; read retries; and the standard quota class (`contract.md` §2.14). The mutation fixtures (`mf_order_*`, `mf_sip_place`, `mf_sip_modify`, `mf_sip_cancel`) and `mf_sip_info.json` have no documented endpoint and are not used |
