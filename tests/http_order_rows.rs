...
async fn envelope_failures_are_the_same_on_every_list_and_policy() {
    ...
    let cases: [(u16, &str, HttpErrorKind, Option<&str>); 9] = [
        (
            ...
            Some("malformed JSON success body: key must be a string at line 1 column 2"),
        ),
        (200, "[1]", HttpErrorKind::Decode, Some("the success body is not a JSON object")),
        (
            200,
            r#"{"data":[]}"#,
            HttpErrorKind::Decode,
        ...
        (
            ...
            HttpErrorKind::Decode,
            Some("the payload does not match the endpoint's type: invalid type: map, expected a sequence"),
        ),
